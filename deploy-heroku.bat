@echo off
REM FightBot Heroku Deployment Script for Windows
...
REM Get app name
set /p APP_NAME="Enter your Heroku app name (or press Enter for 'fightbot-premium'): "
if "%APP_NAME%"=="" set APP_NAME=fightbot-premium

echo 📱 Creating Heroku app: %APP_NAME%

REM Create app (this will fail if it already exists, which is fine)
heroku create %APP_NAME% 2>nul || echo App already exists, continuing...

REM Set app for this directory
...
set /p VARS_SET="Have you set all environment variables? (y/N): "
if /i not "%VARS_SET%"=="y" (
...
git add .
git commit -m "Deploy FightBot to Heroku" 2>nul || echo No changes to commit
