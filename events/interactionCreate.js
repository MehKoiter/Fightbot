...
async function handleSlashCommand(interaction) {
  ...
  try {
    ...
    const commandTimeout = setTimeout(async () => {
      console.warn(
        `⚠️ Command ${interaction.commandName} is taking longer than expected...`
      );
      ...
      if (!interaction.replied && !interaction.deferred) {
        try {
          ...
          console.log(
            `✅ Emergency defer applied for ${interaction.commandName}`
          );
        } catch (deferError) {
          console.error(
            `❌ Failed to apply emergency defer:`,
            deferError.message
          );
        }
      }
    ...
    setImmediate(() => {
      if (interaction.client.userDB) {
        interaction.client.userDB
          .logCommandUsage(interaction.commandName, interaction.guildId)
          .catch((err) =>
            console.error("Background analytics tracking error:", err.message)
          );
      }
    });
  ...
  try {
    ...
    if (command.autocomplete) {
      ...
      const autocompleteTimeout = setTimeout(() => {
        console.warn(
          `⚠️ Autocomplete for ${interaction.commandName} is taking too long...`
        );
  ...
  try {
    ...
    const buttonTimeout = setTimeout(async () => {
      if (
        ...
        !emergencyDeferApplied
      ) {
        try {
          await interaction.deferReply({ ephemeral: true });
          emergencyDeferApplied = true;
          console.log(
            `✅ Emergency defer applied for button ${interaction.customId}`
          );
        } catch (deferError) {
          ...
          if (
            ...
            console.log(
              `⚠️ Emergency defer skipped - interaction ${interaction.customId} has expired`
            );
          } else {
            console.error(
              `❌ Failed to apply emergency defer for button:`,
              deferError.message
            );
          }
        }
    ...
    console.error("Button interaction error:", {
      ...
      isDiscordAPIError:
        error.code === 10062 || error.message.includes("Unknown interaction"),
    });
    ...
    if (error.code === 10062 || error.message.includes("Unknown interaction")) {
      console.log(
        `⚠️ Interaction ${interaction.customId} has expired or already been acknowledged - skipping error response`
      );
      return;
    }
...
async function sendErrorResponse(interaction, response) {
  try {
    ...
    if (
      ...
      responseError.message.includes("Unknown interaction")
    ) {
      console.log(
        `⚠️ Cannot send error response - interaction has expired or been acknowledged`
      );
    } else {
...
async function handleButtonInteraction(interaction) {
  ...
  console.log(
    `🔍 Processing button interaction: ${customId} by ${interaction.user.tag}`
  );
  console.log(
    `🔍 Interaction state - Replied: ${interaction.replied}, Deferred: ${
      interaction.deferred
  ...
  if (customId.startsWith("ufc_")) {
    ...
    console.log(
      `🔍 UFC button - Action: ${action}, DataSource: ${
        dataSource || "legacy"
      }, EventId: ${eventId || "N/A"}`
    );
    ...
    try {
      switch (action) {
        case "details":
          if (!eventId) {
            embed = createSimpleInfoEmbed(
              "❌ Missing Event ID",
              "Event details require a valid event ID."
            );
          } else {
        ...
        case "stats":
          if (!eventId) {
            embed = createSimpleInfoEmbed(
              "❌ Missing Event ID",
              "Event statistics require a valid event ID."
            );
          } else {
            console.log(
              `📊 Creating stats embed for event ${eventId} from ${
                dataSource || "auto-detect"
              }`
            );
      ...
      console.error(
        `❌ Error creating embed for action ${action}:`,
        embedError
      );
  ...
  // Existing fight button interactions
    ...
    switch (action) {
      case "prelims":
        embed = createSimpleInfoEmbed(
          "📺 Preliminary Card",
          "Feature coming soon! All features are completely FREE."
        );
        break;
      case "stats":
        embed = createSimpleInfoEmbed(
          "📊 Fighter Stats",
          "Detailed fighter statistics coming soon! All features are completely FREE."
        );
        break;
      case "refresh":
        embed = createSimpleInfoEmbed(
          "🔄 Data Refreshed",
          "Fight data has been refreshed! All features are completely FREE."
        );
        break;
      default:
        embed = createSimpleInfoEmbed(
  ...
  try {
    ...
    if (dataSource === "sports" && eventId && !isNaN(eventId)) {
      try {
        ...
        console.log(
          `SportsData.io failed for event ${eventId}, trying alternative...`
        );
      }
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(
        `📋 ${
          eventDetails.title || eventDetails.Name || `UFC ${eventId}`
        } - Full Card Details`
      )
      .setDescription(
    ...
    if (fights.length > 0) {
          ...
          .map((fight, index) => {
            if (fight.fighters && fight.fighters.length >= 2) {
              ...
              if (fight.result === "win" || fight.winner) {
                ...
                const loser =
                  fight.fighters.find((f) => f.name !== winner)?.name ||
              ...
              if (
                fight.rawText.includes(" def. ") ||
                fight.rawText.includes(" defeated ")
              ) {
        ...
        fightsList = sortedFights
          .map((fight, index) => {
            ...
            if (fighter1 && fighter2) {
              ...
              if (
                fight.Status === "Final" ||
                fighter1.Winner !== undefined ||
                fighter2.Winner !== undefined
              ) {
                let winner, loser;
                ...
                if (winner && loser) {
                  ...
                  fight.Status === "Final" &&
                  fight.ResultType &&
                  fight.ResultType.toLowerCase().includes("draw")
                ) {
                  // Handle draws
      ...
      chunks.forEach((chunk, index) => {
        embed.addFields({
          name:
            index === 0
              ? `🥊 Complete Fight Card & Results (${fights.length} fights)`
  ...
  try {
    ...
    if (dataSource === "sports" && eventId && !isNaN(eventId)) {
      try {
        ...
        console.log(
          `SportsData.io failed for event ${eventId}, trying Wikipedia...`
        );
      }
    } else if (dataSource === "wiki") {
      try {
    ...
    if (fights.length > 0) {
      ...
      embed.addFields({
        name: "🎭 Card Breakdown",
        value: `**Main Card:** ${mainCardCount} fights\n**Preliminary Card:** ${prelimsCount} fights${
          earlyPrelimsCount > 0
...
async function createUpcomingEventsEmbed() {
  try {
    ...
    eventsToShow.forEach((event, index) => {
      ...
      const location =
        event.City && event.Country ? `${event.City}, ${event.Country}` : "TBA";
    ...
    embed.addFields({
      name: "💡 Tip",
      value:
        "Use `/ufc [number]` to get detailed information about a specific event!",
      inline: false,
    });
