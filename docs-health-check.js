...
function checkLinksInFile(filePath) {
  ...
  while ((match = linkRegex.exec(content)) !== null) {
    ...
    if (
      linkPath.includes("category/DOCUMENT.md") ||
      linkPath.includes("Section")
    ) {
      continue;
    }
    ...
    if (linkPath.endsWith("/")) {
      ...
      if (
        ...
        !fs.statSync(absoluteLinkPath).isDirectory()
      ) {
        brokenLinks.push({
...
function generateReport() {
  ...
  if (results.outdatedFiles.length === 0) {
    ...
    console.log(
      `⚠️  Potentially Outdated Files: ${results.outdatedFiles.length}`
    );
    for (const outdated of results.outdatedFiles) {
      console.log(
        `   ${outdated.file} (${outdated.daysSinceModified} days old)`
      );
    }
  }
