...
export default {
  data: new SlashCommandBuilder()
    .setName("fight")
    .setDescription(
      "Get information about the upcoming UFC event with fight card details"
    ),
    ...
    try {
      ...
      if (!event) {
        const errorEmbed = new EmbedBuilder()
          ...
          .setDescription(
            "Sorry, I couldn't find any upcoming UFC events at the moment."
          )
          .addFields({
            name: "🔧 Possible Reasons",
            value:
              "• UFC.com may be temporarily unavailable\n• No events currently scheduled\n• Network connection issues",
            inline: false,
          })
      ...
      const eventEmbed = new EmbedBuilder()
        ...
        .setFooter({
          ...
          iconURL:
            "https://logoeps.com/wp-content/uploads/2013/03/ufc-vector-logo.png",
        })
        .setTimestamp();
      ...
      if (event.fights && event.fights.length > 0) {
        ...
        const validFights = event.fights.filter(
          (fight) =>
            fight.redCorner?.name &&
            fight.redCorner.name !== "TBA" &&
            fight.blueCorner?.name &&
            fight.blueCorner.name !== "TBA"
        );
        ...
        if (validFights.length === 0) {
          ...
          const errorEmbed = new EmbedBuilder()
            ...
            .setDescription(
              "The fight card details are not yet available for this event."
            )
            .addFields({
              name: "🔧 This could mean:",
              value:
                "• Fight card hasn't been announced yet\n• UFC website structure has changed\n• Event details are being updated",
              inline: false,
            })
            .setFooter({
              text: "Check back later or visit UFC.com for updates",
            })
            .setTimestamp();
        ...
        const noFightsEmbed = new EmbedBuilder()
          ...
          .setTitle("⚠️ Fight Card Not Available")
          .setDescription(
            "Fight card information could not be retrieved for this event."
          )
          .addFields({
            name: "🔧 Possible reasons:",
            value:
              "• Fight card not yet announced\n• Website parsing issues\n• Event page structure changed",
            inline: false,
          })
      ...
      const errorEmbed = new EmbedBuilder()
        ...
        .setTitle("❌ Error")
        .setDescription(
          "Sorry, there was an error fetching the fight information. Please try again in a moment."
        )
        .addFields({
          name: "🔧 Troubleshooting",
          value:
            "• Check your internet connection\n• Try the command again\n• The UFC website may be temporarily unavailable",
          inline: false,
        })
      ...
      try {
        ...
        if (interaction.deferred && !interaction.replied) {
          ...
          console.log(
            "⚠️ Fight command error: Cannot respond - interaction already handled"
          );
        }
      } catch (replyError) {
        ...
        if (replyError.code === 10062) {
          console.error(
            "❌ Failed to send error message: Unknown interaction (already expired)"
          );
        } else if (replyError.message.includes("already been acknowledged")) {
          console.error(
            "❌ Failed to send error message: Interaction already acknowledged"
          );
        } else {
