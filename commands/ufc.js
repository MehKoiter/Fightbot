        ...
        try {
            ...
            if (!cleanEventNumber || cleanEventNumber.length === 0) {
                const errorEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        { name: '💡 Examples', value: '• `/ufc 199`\n• `/ufc 309`\n• `/ufc UFC 285`', inline: false }
                    )
            ...
            if (!event) {
                console.log(`🔄 Wikipedia didn't find UFC ${cleanEventNumber}, trying SportsData.io...`);
            ...
            if (!event) {
                const notFoundEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`Sorry, I couldn't find information for UFC ${cleanEventNumber}.`)
                    .addFields(
                        { name: '🔍 Possible Reasons', value: '• Event number doesn\'t exist\n• Event is too old (only recent years are available)\n• Event may not be in our database yet', inline: false },
                        { name: '💡 Try', value: '• Check the event number is correct\n• Try a more recent UFC event\n• Use `/fight` for upcoming events', inline: false }
                    )
            ...
            const eventEmbed = new EmbedBuilder()
                .setColor('#ff6600')
                .setTitle(`🥊 ${event.title || event.Name || `UFC ${cleanEventNumber}`}`)
                .setDescription(event.description || event.ShortName || `UFC ${cleanEventNumber} Event Details`);
            ...
            const location = event.location || event.venue || (event.City && event.Country ? `${event.City}, ${event.Country}` : null);
            if (location) {
                eventEmbed.addFields({
                    name: '📍 Location',
                    value: location,
            ...
            if (fights && fights.length > 0) {
                ...
                if (dataSource === 'Wikipedia') {
                    // Wikipedia format
                    fightCardText = fights.slice(0, 8).map((fight, index) => {
                    ...
                    // SportsData format
                    const mainCard = fights.filter(fight => 
                        fight.Card === 'Main' || fight.Order <= 5
                    ).sort((a, b) => (b.Order || 0) - (a.Order || 0));

                    fightCardText = mainCard.slice(0, 8).map((fight, index) => {
            ...
            const errorEmbed = new EmbedBuilder()
                ...
                .setDescription('Sorry, something went wrong while fetching the UFC event information.')
                .addFields(
                    { name: '🔧 Possible Issues', value: '• SportsData.io API may be temporarily unavailable\n• Network connection issues\n• Service timeout', inline: false }
                )
