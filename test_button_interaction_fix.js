...
async function testNormalUFCButton() {
    ...
    try {
        ...
        if (customId.startsWith('ufc_')) {
            ...
            console.log(`   Action: ${action}, DataSource: ${dataSource}, EventId: ${eventId}`);
...
async function testAlreadyDeferredButton() {
    ...
    try {
        const interaction = createMockInteraction('ufc_stats_wiki_234', { deferred: true });
...
async function testExpiredInteractionDefer() {
    ...
    try {
        const interaction = createMockInteraction('ufc_stats_wiki_234', { shouldFailDefer: true });
        ...
        if (customId.startsWith('ufc_')) {
            try {
                ...
                if (deferError.code === 10062 || deferError.message.includes('Unknown interaction')) {
                    console.log('   Detected expired interaction - throwing Unknown interaction error');
