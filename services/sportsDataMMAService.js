...
export default class SportsDataMMAService {
  constructor() {
    ...
    if (!this.apiKey) {
      console.warn(
        "⚠️ SportsData.io API key not configured. Set SPORTSDATA_API_KEY in your .env file"
      );
    }
  }
  ...
  async getAllFightersBasic() {
    try {
      ...
      const response = await axios.get(
        `${this.baseUrl}/scores/json/FightersBasic`,
        {
          timeout: 15000,
          headers: {
      ...
      if (response.status === 200 && response.data) {
        ...
        console.log(
          `✅ Retrieved ${response.data.length} basic fighter profiles`
        );
  ...
  async getFighterProfile(fighterId) {
    try {
      ...
      console.log(
        `🔍 SportsData.io: Fetching fighter profile for ID: ${fighterId}`
      );

      const response = await axios.get(
        `${this.baseUrl}/scores/json/Fighter/${fighterId}`,
        {
          timeout: 10000,
          headers: {
            "Ocp-Apim-Subscription-Key": this.apiKey,
      ...
      if (response.status === 200 && response.data) {
        ...
        console.log(
          `✅ Retrieved detailed profile for: ${response.data.FirstName} ${response.data.LastName}`
        );
        return response.data;
      }
      ...
      console.error(
        `❌ Error fetching fighter profile for ID ${fighterId}:`,
        error.message
      );
      return null;
    }
  ...
  async getUFCSchedule(season = "2025") {
    try {
      ...
      const response = await axios.get(
        `${this.baseUrl}/scores/json/Schedule/UFC/${season}`,
        {
          timeout: 10000,
          headers: {
            "Ocp-Apim-Subscription-Key": this.apiKey,
      ...
      if (response.status === 200 && response.data) {
        ...
        console.log(
          `✅ Retrieved ${response.data.length} UFC events for ${season}`
        );
        return response.data;
      }
      ...
      console.error(
        `❌ Error fetching UFC schedule for ${season}:`,
        error.message
      );
      return [];
    }
  ...
  async getEventDetails(eventId) {
    try {
      ...
      console.log(
        `🔍 SportsData.io: Fetching event details for ID: ${eventId}`
      );

      const response = await axios.get(
        `${this.baseUrl}/scores/json/Event/${eventId}`,
        {
          timeout: 10000,
          headers: {
            "Ocp-Apim-Subscription-Key": this.apiKey,
      ...
      console.error(
        `❌ Error fetching event details for ID ${eventId}:`,
        error.message
      );
      return null;
    }
  ...
  async getFightDetails(fightId) {
    try {
      ...
      console.log(
        `🔍 SportsData.io: Fetching fight details for ID: ${fightId}`
      );

      const response = await axios.get(
        `${this.baseUrl}/stats/json/Fight/${fightId}`,
        {
          timeout: 10000,
          headers: {
            "Ocp-Apim-Subscription-Key": this.apiKey,
      ...
      console.error(
        `❌ Error fetching fight details for ID ${fightId}:`,
        error.message
      );
      return null;
    }
  ...
  async compareFighters(fighter1Name, fighter2Name) {
    try {
      console.log(
        `🔍 SportsData.io: Comparing ${fighter1Name} vs ${fighter2Name}`
      );
      ...
      console.log(
        `✅ Successfully compared ${fighter1Name} vs ${fighter2Name}`
      );
      return comparison;
    } catch (error) {
  ...
  async getUFCEventByNumber(eventNumber) {
    try {
        ...
        try {
          ...
          if (schedule && schedule.length > 0) {
            ...
            const matchingEvent = schedule.find((event) => {
              ...
              return (
                eventName.includes(searchPattern) ||
            ...
            if (matchingEvent) {
              ...
              if (eventDetails) {
                ...
                console.log(
                  `✅ Found UFC ${eventNumber}: ${eventDetails.Name}`
                );
                return eventDetails;
              }
            }
          ...
          if (yearError.response?.status === 401) {
            console.log(
              `⚠️ Limited API access for year ${year}, continuing search...`
            );
            continue;
          }
          throw yearError;
      ...
      console.error(
        `❌ Error searching for UFC ${eventNumber}:`,
        error.message
      );
      return null;
    }
