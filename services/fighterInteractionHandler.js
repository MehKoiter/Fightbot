...
export default class FighterInteractionHandler {
    ...
    async handleDetailedStats(interaction) {
            ...
            .addFields(
                {
                    ...
                    value: `**Significant Strikes Landed:** ${fighter.fightingStyle.striking.avgPerMinute}/min\n` +
                           `**Striking Accuracy:** ${fighter.fightingStyle.striking.accuracy}\n` +
                           `**Striking Defense:** ${fighter.fightingStyle.striking.defense}\n` +
                           `**Knockdown Ratio:** ${(fighter.record.winsByKO / fighter.record.wins * 100).toFixed(1)}%`,
                    inline: true
                },
                {
                    name: '🤼 Grappling Analytics',
                    value: `**Takedown Accuracy:** ${fighter.fightingStyle.grappling.takedownAccuracy}\n` +
                           ...
                           `**Submission Rate:** ${(fighter.record.winsBySubmission / fighter.record.wins * 100).toFixed(1)}%`,
                    inline: true
                },
                {
                    name: '🏆 Career Statistics',
                    value: `**Total Fights:** ${fighter.record.wins + fighter.record.losses + fighter.record.draws}\n` +
                           `**Win Percentage:** ${((fighter.record.wins / (fighter.record.wins + fighter.record.losses)) * 100).toFixed(1)}%\n` +
                           `**Finish Rate:** ${(((fighter.record.winsByKO + fighter.record.winsBySubmission) / fighter.record.wins) * 100).toFixed(1)}%\n` +
                           `**Decision Rate:** ${((fighter.record.winsByDecision / fighter.record.wins) * 100).toFixed(1)}%`,
                    inline: false
                },
                {
                    name: '🎯 Fighting Style Analysis',
                    value: `**Primary Stance:** ${fighter.physicalStats.stance}\n` +
        ...
        await interaction.editReply({ embeds: [statsEmbed], components: [backButton] });
    }
            ...
            .addFields(
                {
                    ...
                    value: fighter.highlights.map((highlight, index) => `${index + 1}. ${highlight}`).join('\n') || 'No highlights available',
                    inline: false
                },
                {
                    name: '🏆 Career Achievements',
                    value: fighter.achievements.slice(0, 5).map((achievement, index) => `• ${achievement}`).join('\n') || 'Professional Fighter',
                    inline: false
                },
                {
                    name: '📱 Follow on Social Media',
                    value: `**Instagram:** ${fighter.socialMedia.instagram}\n` +
        ...
        const backButton = new ActionRowBuilder()
            .addComponents(
                ...
                new ButtonBuilder()
                    ...
                    .setURL(`https://www.ufc.com/athlete/${fighterName.toLowerCase().replace(' ', '-')}`)
            );

        await interaction.editReply({ embeds: [highlightsEmbed], components: [backButton] });
    }
    ...
    async handleComparePrompt(interaction) {
        ...
        const promptEmbed = new EmbedBuilder()
            ...
            .setDescription(`To compare **${fighterName}** with another fighter, use the command:\n\n` +
                           `\`/fighter name:${fighterName} compare:[fighter name]\`\n\n` +
                           `**Example:**\n` +
                           `\`/fighter name:${fighterName} compare:Jon Jones\``)
            .addFields({
                name: '💡 Pro Tip',
                value: 'Use the autocomplete feature to quickly find fighters to compare!',
                inline: false
            })
            .setTimestamp()
    ...
    async handleRefreshProfile(interaction) {
        ...
        const { createFighterEmbed, createActionButtons } = await import('./fighter.js');
    ...
    async handleDetailedComparison(interaction) {
        ...
        const fighters = interaction.customId.replace('comparison_detailed_', '').split('_vs_');
        const [fighter1Name, fighter2Name] = fighters;

        const comparison = await this.fighterService.compareFighters(fighter1Name, fighter2Name);
        ...
        const detailedEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                },
                {
                    ...
                    value: comp.physical.heightAdvantage !== 0 ? 
                           `**Height:** ${comp.physical.heightAdvantage > 0 ? fighter1.name : fighter2.name} +${Math.abs(comp.physical.heightAdvantage)}"\n` :
                           '**Height:** Even matchup\n' +
                           (comp.physical.reachAdvantage !== 0 ? 
                           `**Reach:** ${comp.physical.reachAdvantage > 0 ? fighter1.name : fighter2.name} +${Math.abs(comp.physical.reachAdvantage).toFixed(1)}"` :
                           '**Reach:** Even matchup'),
                    inline: false
                }
            )
    ...
    async handleFightingStyles(interaction) {
        ...
        const fighters = interaction.customId.replace('comparison_styles_', '').split('_vs_');
        const [fighter1Name, fighter2Name] = fighters;
            ...
            .addFields(
                {
                    ...
                    value: `**Stance:** ${fighter1.physicalStats.stance}\n` +
                           `**Striking Style:** ${parseFloat(fighter1.fightingStyle.striking.accuracy) > 60 ? 'Accurate Striker' : 'Volume Striker'}\n` +
                           `**Grappling Style:** ${parseFloat(fighter1.fightingStyle.grappling.takedownAccuracy) > 50 ? 'Takedown Heavy' : 'Defensive Grappler'}\n` +
                           `**Finish Rate:** ${(((fighter1.record.winsByKO + fighter1.record.winsBySubmission) / fighter1.record.wins) * 100).toFixed(1)}%`,
                    inline: true
                },
                {
                    name: `🎯 ${fighter2.name}'s Style`,
                    value: `**Stance:** ${fighter2.physicalStats.stance}\n` +
                           `**Striking Style:** ${parseFloat(fighter2.fightingStyle.striking.accuracy) > 60 ? 'Accurate Striker' : 'Volume Striker'}\n` +
                           `**Grappling Style:** ${parseFloat(fighter2.fightingStyle.grappling.takedownAccuracy) > 50 ? 'Takedown Heavy' : 'Defensive Grappler'}\n` +
                           `**Finish Rate:** ${(((fighter2.record.winsByKO + fighter2.record.winsBySubmission) / fighter2.record.wins) * 100).toFixed(1)}%`,
                    inline: true
                },
                {
                    name: '⚔️ Style Matchup Analysis',
                    value: `This analysis shows how the fighters' styles might interact in a potential matchup.\n\n` +
                           `**Key Factors:**\n` +
    ...
    async handleFightPrediction(interaction) {
        ...
        const fighters = interaction.customId.replace('comparison_prediction_', '').split('_vs_');
        const [fighter1Name, fighter2Name] = fighters;
        ...
        const f1Score = this.calculateFighterScore(fighter1, comp.advantages.fighter1);
        const f2Score = this.calculateFighterScore(fighter2, comp.advantages.fighter2);
        ...
        const predictionEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                {
                    name: '📊 Key Factors',
                    value: `**${favorite.name} Advantages:**\n` +
                           `${comp.advantages[f1Score > f2Score ? 'fighter1' : 'fighter2'].slice(0, 3).join('\n') || 'Experience'}\n\n` +
                           `**${underdog.name} Path to Victory:**\n` +
                           `${comp.advantages[f1Score > f2Score ? 'fighter2' : 'fighter1'].slice(0, 2).join('\n') || 'Upset potential'}`,
                    inline: true
                },
                {
                    name: '⚠️ Disclaimer',
                    value: 'This prediction is based on available statistics and should be used for entertainment purposes only. MMA is unpredictable!',
                    inline: false
                }
            )
            .setTimestamp()
    ...
    calculateFighterScore(fighter, advantages) {
        ...
        // Record score
        const winRate = (fighter.record.wins / (fighter.record.wins + fighter.record.losses)) * 100;
    ...
    predictMethod(fighter) {
        ...
        const subRate = (fighter.record.winsBySubmission / fighter.record.wins) * 100;
