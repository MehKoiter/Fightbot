...
class UserDatabaseService {
  constructor() {
    try {
      ...
      console.error(
        "❌ Error in UserDatabaseService constructor:",
        error.message
      );
      // Set fallback values
      this.dbPath = ":memory:";
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        try {
          this.db = new sqlite3.Database(this.dbPath, (err) => {
            if (err) {
              ...
              this.db = new sqlite3.Database(":memory:", (memErr) => {
                if (memErr) {
                  console.error(
                    "❌ In-memory database failed:",
                    memErr.message
                  );
                  this.isInitialized = false;
                  reject(
                    new Error(
                      `Database initialization failed: ${memErr.message}`
                    )
                  );
                } else {
                  ...
                  this.createTables()
                    .then(() => {
                      console.log(
                        "✅ Database initialization completed (in-memory)"
                      );
                    ...
                    .catch((tableErr) => {
                      ...
                      this.isInitialized = false;
                      reject(
                        new Error(`Table creation failed: ${tableErr.message}`)
                      );
                    });
                }
              });
          ...
          this.isInitialized = false;
          reject(
            new Error(`Database creation failed: ${dbCreateError.message}`)
          );
        }
      });
    } catch (error) {
        ...
        (err, totalRow) => {
          ...
          this.db.all(
            ...
            (err, rows) => {
              if (err) {
                console.warn(
                  "⚠️ Failed to get command breakdown:",
                  err.message
                );
                resolve({
