...
export default class WikipediaUFCService {
  ...
  async getUFCEventByNumber(eventNumber) {
    try {
      ...
      for (const title of possibleTitles) {
        const eventData = await this.fetchUFCEventFromWikipedia(
          title,
          eventNumber
        );
        if (eventData) {
          ...
          console.log(
            `✅ Found UFC ${eventNumber} on Wikipedia: ${eventData.title}`
          );
          return eventData;
        }
      }
      ...
      console.error(
        `❌ Error searching Wikipedia for UFC ${eventNumber}:`,
        error.message
      );
      return null;
    }
  ...
  async fetchUFCEventFromWikipedia(pageTitle, eventNumber) {
    try {
      ...
      console.error(
        `❌ Error fetching Wikipedia page ${pageTitle}:`,
        error.message
      );
      return null;
    }
  ...
  async getFightCardFromWikipedia(pageTitle) {
    try {
      ...
      if (
        !sectionsResponse.data.parse ||
        !sectionsResponse.data.parse.sections
      ) {
        return null;
      }
      ...
      console.error(
        "❌ Error fetching fight card from Wikipedia:",
        error.message
      );
      return null;
    }
  ...
  parseFightCardHTML(html) {
    try {
      ...
      $("table").each((i, table) => {
        ...
        if (
          tableText.includes("fight") ||
          tableText.includes("vs") ||
          tableText.includes("def.") ||
          tableText.includes("defeated")
        ) {
          $table.find("tr").each((rowIndex, row) => {
            ...
            // Skip header rows
            if (
              rowText.toLowerCase().includes("weight") &&
              rowText.toLowerCase().includes("method")
            ) {
              return;
            }
            ...
            if (
              rowText.includes(" vs ") ||
              rowText.includes(" def. ") ||
              rowText.includes(" defeated ") ||
              rowText.includes("def.")
            ) {
              const cells = $row
                .find("td, th")
      ...
      if (fights.length < 5) {
        $("p").each((i, p) => {
          ...
          boldTexts.forEach((text) => {
            if (
              (text.includes(" vs ") || text.includes(" def. ")) &&
              text.length < 200
            ) {
              const fightData = this.extractFightFromText(text);
              if (fightData && fightData.fighters.length >= 2) {
                ...
                const isDuplicate = fights.some((existingFight) =>
                  existingFight.fighters.some((f1) =>
                    fightData.fighters.some(
                      (f2) => f1.name.toLowerCase() === f2.name.toLowerCase()
                    )
                  )
                );
    ...
    try {
      ...
      if (defIndex > 0 && defIndex < cells.length - 1) {
        ...
        if (winner && loser && winner.length > 1 && loser.length > 1) {
          return {
            fighters: [{ name: winner }, { name: loser }],
            winner: winner,
            result: "win",
  ...
  extractFightFromText(text) {
    try {
      ...
      const weightClassMatch = text.match(
        /(Heavyweight|Light Heavyweight|Middleweight|Welterweight|Lightweight|Featherweight|Bantamweight|Flyweight|Women's)/i
      );
      if (weightClassMatch) {
      ...
      const methodMatch = text.match(
        /(KO|TKO|Submission|Decision|Unanimous Decision|Majority Decision|Split Decision|DQ|NC|Technical Decision)/i
      );
      if (methodMatch) {
      ...
      fightData.fighters = fightData.fighters.filter(
        (fighter) =>
  ...
  cleanFighterName(name) {
    return name
      ...
      .replace(
        /Heavyweight|Light Heavyweight|Middleweight|Welterweight|Lightweight|Featherweight|Bantamweight|Flyweight|Women's/gi,
        ""
      ) // Remove weight classes
