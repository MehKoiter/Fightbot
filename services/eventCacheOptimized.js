...
class EventCache {
  ...
  set(key, eventData) {
    ...
    try {
      ...
      if (dataSize > 5 * 1024 * 1024) {
        // 5MB limit per entry
        console.warn(
          `⚠️ Cache entry too large (${(dataSize / 1024 / 1024).toFixed(
            2
          )}MB), skipping: ${key}`
        );
        return false;
  ...
  checkMemoryPressure() {
    ...
    if (totalSize > this.maxMemorySize) {
      console.warn(
        `💾 Cache memory pressure: ${(totalSize / 1024 / 1024).toFixed(
          2
        )}MB, cleaning up...`
      );
  ...
  memoryPressureCleanup() {
    ...
    console.log(
      `🧹 Memory pressure cleanup: removed ${
  ...
  aggressiveCleanup() {
    ...
    console.log(
      `🆘 Aggressive cleanup: kept ${keepCount}/${beforeSize} entries`
    );
  }
  ...
  getStats() {
    ...
    return {
      ...
      memoryUtilization: `${((totalSize / this.maxMemorySize) * 100).toFixed(
        1
      )}%`,
    };
  }
