...
export default class FighterService {
  ...
  generateComparison(fighter1, fighter2) {
    return {
      physical: {
        heightAdvantage: this.calculateHeightAdvantage(
          fighter1.physicalStats.height,
          fighter2.physicalStats.height
        ),
        reachAdvantage: this.calculateReachAdvantage(
          fighter1.physicalStats.reach,
          fighter2.physicalStats.reach
        ),
      ...
      experience: {
        totalFights:
          ...
          fighter1.record.losses -
          (fighter2.record.wins + fighter2.record.losses),
        winPercentage:
          this.calculateWinPercentage(fighter1.record) -
          this.calculateWinPercentage(fighter2.record),
      },
      advantages: {
  ...
  getFighterAdvantages(fighter, opponent) {
    ...
    // Physical advantages
    if (
      this.calculateHeightAdvantage(
        fighter.physicalStats.height,
        opponent.physicalStats.height
      ) > 2
    ) {
      advantages.push("Height advantage");
    }
    if (
      this.calculateReachAdvantage(
        fighter.physicalStats.reach,
        opponent.physicalStats.reach
      ) > 2
    ) {
    ...
    if (
      parseFloat(fighter.fightingStyle.striking.accuracy) >
      parseFloat(opponent.fightingStyle.striking.accuracy)
    ) {
