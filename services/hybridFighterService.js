...
export default class HybridFighterService {
    ...
    async getFighterProfile(fighterName) {
        try {
            ...
            try {
                const sportsDataFighters = await this.sportsDataService.searchFighters(fighterName);
                if (sportsDataFighters && sportsDataFighters.length > 0) {
                    const detailedProfile = await this.sportsDataService.getFighterProfile(sportsDataFighters[0].FighterId);
                    if (detailedProfile) {
                        fighterProfile = this.formatSportsDataProfile(detailedProfile);
                        fighterProfile.source = 'SportsData.io API';
                        console.log(`✅ Found fighter via SportsData.io: ${fighterProfile.name}`);
                    }
                }
            } catch (error) {
            ...
            if (!fighterProfile) {
                try {
                    const ufcProfile = await this.ufcStatsService.getFighterProfile(fighterName);
                    if (ufcProfile) {
                        fighterProfile = this.formatUFCStatsProfile(ufcProfile);
                        fighterProfile.source = 'UFC.com Scraping';
                        console.log(`✅ Found fighter via UFC scraping: ${fighterProfile.name}`);
                    }
                } catch (error) {
            ...
            if (!fighterProfile) {
                try {
                    const basicProfile = await this.basicFighterService.getFighterProfile(fighterName);
                    if (basicProfile) {
                        fighterProfile = this.formatBasicProfile(basicProfile);
                        fighterProfile.source = 'Basic/Mock Data';
                        console.log(`✅ Found fighter via basic service: ${fighterProfile.name}`);
                    }
                } catch (error) {
            ...
            console.error(`❌ Error in hybrid fighter lookup for ${fighterName}:`, error.message);
            return null;
        }
    }
    ...
    async searchFighters(query) {
        try {
            ...
            try {
                const sportsDataResults = await this.sportsDataService.searchFighters(query);
                if (sportsDataResults && sportsDataResults.length > 0) {
                    console.log(`✅ Found ${sportsDataResults.length} fighters via SportsData.io`);
            ...
            try {
                ...
                if (ufcResults && ufcResults.length > 0) {
                    console.log(`✅ Found ${ufcResults.length} fighters via UFC scraping`);
            ...
            // Final fallback to basic service
            try {
                const basicResults = await this.basicFighterService.searchFighter(query);
                if (basicResults && basicResults.length > 0) {
                    console.log(`✅ Found ${basicResults.length} fighters via basic service`);
    ...
    async compareFighters(fighter1Name, fighter2Name) {
        try {
            ...
            try {
                const sportsDataComparison = await this.sportsDataService.compareFighters(fighter1Name, fighter2Name);
            ...
            try {
                const ufcComparison = await this.ufcStatsService.compareFighters(fighter1Name, fighter2Name);
                if (ufcComparison) {
    ...
    async getAutocompleteSuggestions(query) {
        try {
            ...
            // Try SportsData.io first
            try {
                const sportsDataSuggestions = await this.sportsDataService.getAutocompleteSuggestions(query);
            ...
            // Fallback to UFC Stats
            try {
                const ufcSuggestions = await this.ufcStatsService.getAutocompleteSuggestions(query);
    ...
    formatSportsDataProfile(fighter) {
        return {
            ...
            details: {
                ...
                birthPlace: fighter.BirthCity && fighter.BirthCountry ? 
                    `${fighter.BirthCity}, ${fighter.BirthCountry}` : 'N/A',
    ...
    compareProfiles(fighter1, fighter2) {
        ...
        if (fighter1.record && fighter2.record) {
            if (typeof fighter1.record === 'object' && typeof fighter2.record === 'object') {
