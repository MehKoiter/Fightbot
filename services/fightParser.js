...
export class Fight {
  constructor(
    redCorner = new FightCorner(),
    blueCorner = new FightCorner(),
    weightClass = ""
  ) {
...
export class FightParser {
  ...
  parseEvents(html) {
    ...
    parsedHTML(
      ".c-card-event--result__headline a, .c-card-event__headline a"
    ).each((_, element) => {
  ...
  tryFighterSelectors(cheerioApi) {
    ...
    for (const selector of fighterSelectors) {
      const fighters = cheerioApi(selector)
        .map((_, el) =>
      ...
      if (fighters.length > 0) {
        console.log(
          `✅ Found ${fighters.length} fighters with selector: ${selector}`
        );
        return fighters;
      }
    }
  ...
  tryWeightClassSelectors(cheerioApi) {
    ...
    for (const selector of weightSelectors) {
      const weights = cheerioApi(selector)
        .map((_, el) =>
          cheerioApi(el).text().trim().replace(/\n/g, "").replace(/\s+/g, " ")
        )
        .get()
        .filter((weight) => weight && weight.length > 0);

      if (weights.length > 0) {
        console.log(
          `✅ Found ${weights.length} weight classes with selector: ${selector}`
        );
        return weights;
      }
    }
  ...
  tryRankSelectors(cheerioApi) {
    ...
    for (const selector of rankSelectors) {
      const ranks = cheerioApi(selector)
        .map((_, el) => cheerioApi(el).text().trim().replace(/\n/g, ""))
        .get();

      if (ranks.length > 0) {
        console.log(
          `✅ Found ${ranks.length} ranks with selector: ${selector}`
        );
        return ranks;
      }
    }
    ...
    console.log(
      "ℹ️ No ranks found with any selector (this is normal for some events)"
    );
    return [];
  }
  /**
  ...
  parseEvent(html) {
    ...
    if (fighters.length === 0 && weightClasses.length === 0) {
      console.log(
        "❌ No fighters or weight classes found - parsing failed completely"
      );
    ...
    for (
      let i = 0;
      i < weightClasses.length && i * 2 + 1 < fighters.length;
      i++
    ) {
    ...
    const subtitle = parsedHTML(subtitleClass)
      .text()
      .trim()
      .replace(/\n/g, "")
      .replace(/ +/g, " ");
    ...
    if (date) {
      ...
      const timeMatch = date.match(
        /(\d{1,2}:\d{2}\s*(?:AM|PM)?\s*(?:EDT|EST|ET|PDT|PST|PT|GMT|UTC)?)/i
      );
      if (timeMatch) {
  ...
  tryLocationSelectors(cheerioApi) {
    ...
    for (const selector of locationSelectors) {
      ...
      if (
        ...
        console.log(
          `📍 Found location with selector ${selector}:`,
          rawLocation
        );
        return rawLocation;
      }
