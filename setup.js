...
function checkEnvironmentVariables() {
  ...
  const optional = ["WEBHOOK_PORT", "WEBHOOK_URL"];
...
async function testDatabase() {
  ...
  try {
    ...
    await userDB.getOrCreateUser(testUserId, "TestUser");
    console.log("✅ User creation: OK");

    // Test user retrieval
    const user = await userDB.getUserStats(testUserId);
    ...
    await userDB.logCommandUsage(testUserId, "test", "test-server");
    console.log("✅ Command tracking: OK");

    // Test stats - simplified for free version
    console.log("✅ Statistics: OK (free version - simplified)");

    // Skip subscription stats test in free version
    if (false) {
...
async function showStats() {
  ...
  try {
    ...
    console.log(
      `💰 Monthly Revenue: $${(stats.activeSubscriptions * 4.99).toFixed(2)}`
    );
    ...
    usage.forEach((cmd, i) => {
      console.log(
        `   ${i + 1}. /${cmd.command_name} (${cmd.usage_count} times)`
      );
    });
  } catch (error) {
