...
async function testUFCStatsEmbed() {
    ...
    console.log(`Event: ${eventDetails.title || eventDetails.Name || 'UFC ' + eventId}`);
    console.log(`Total Fights: ${fights.length}`);
    console.log(`Date: ${eventDetails.dateTime || eventDetails.DateTime || eventDetails.date}`);
    console.log(`Venue: ${eventDetails.venue || eventDetails.location}`);
    
    if (fights.length > 0) {
        console.log('\nFirst 3 fights:');
        fights.slice(0, 3).forEach((fight, i) => {
            const name1 = fight.fighters?.[0]?.name || fight.Fighter1?.Name || 'Unknown';
            const name2 = fight.fighters?.[1]?.name || fight.Fighter2?.Name || 'Unknown';
